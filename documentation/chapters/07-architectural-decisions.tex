...
QA Scenario 1 (Availability under Warehouse Failure) requires that a warehouse timeout or
rejection does not prevent a web order from being recorded. Scenario 4 (Recoverability after
...
entirely. Scenario 1 rules this out.
...
transaction. A scheduled task periodically reads unpublished records, dispatches them to the
integration layer, and marks them published. If the process restarts before the scheduled task
...
idempotent, duplicate messages will arrive during retries after uncertain acknowledgements.
...
outbox moves the broker dependency to the background scheduled task, which tolerates broker
unavailability at commit time without affecting the customer.

\subsection{Consequences}

No paid order is silently dropped due to a transient broker or process failure. The scheduled
task introduces a small propagation delay between order creation and warehouse notification,
...
remain within the bounds set by QA Scenario 1.
...
QA Scenario 5 (Modifiability for New Operational Channels) requires that a new system can be
...
customer accounts, and canonical order management stay inside the monolith. The only
independently deployable component introduced by this evolution is the Integration Coordination
context, which owns the outbox, retry, idempotency, and reconciliation behaviour described in
ADR 2. Adapters for warehouse, shipping, inventory, and store operations sit around this layer.
The monolith interacts with the Integration Coordination context through its outbox table and a
publishing scheduled task, remaining decoupled from the internal design of any individual
adapter.
...
availability requirement in QA Scenario 1. The decomposition adds complexity without improving
any of the measured quality attributes.

\textbf{Extract inventory as an independent service.} Given QA Scenario 2 (Consistency under
...
catalog, admin, and plugin extensibility intact. The Integration Coordination context is the
only new independently deployable component, and its scope is tightly bounded to reliability
and coordination. The main limitation is that the commerce core cannot be scaled independently
from the integration layer, but horizontal scaling of the combined unit is sufficient for the
load levels identified in QA Scenario 3 and is not a constraint that justifies decomposition at
this stage.
