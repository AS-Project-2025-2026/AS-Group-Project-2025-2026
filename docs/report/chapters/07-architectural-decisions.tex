...
dead-letter is approximately 50 minutes; circuit-breaker state must be surfaced in the
Support and Operations View and monitored with alerting; backoff parameters introduce
configuration that must be validated during testing. This decision should be revisited if
the 50-minute dead-letter threshold is too slow for a particular adapter's recovery
characteristic.
...
\textbf{Dominant forces:} supportability (operators must be able to observe, acknowledge,
and act on permanent failures), data consistency (a paid order must never be silently
abandoned), operational complexity (human intervention must be guided by clear escalation
state and UI affordances).
...
as critical infrastructure; the 30-day retention creates a storage obligation. This decision
should be revisited if the volume of dead-letter entries grows to a level that makes manual
review unsustainable, at which point automated triage or a tiered escalation model may be
needed.
