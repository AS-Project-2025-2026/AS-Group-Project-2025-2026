...
\textbf{Dominant forces:} fault isolation (checkout must not depend on warehouse availability),
data consistency (fulfilled orders must not be silently lost), supportability (operators must
observe pending and retrying states).

\textbf{Accepted damage:} fulfillment state is eventually consistent rather than immediately
visible; the order model requires intermediate lifecycle states; idempotency infrastructure
is mandatory (ADR~5). This decision should be revisited if propagation delay consistently
exceeds the bounds set by QAS~1 under normal operating conditions.
...
\textbf{Dominant forces:} data consistency (order and integration task must be atomic),
fault isolation (broker unavailability at commit time must not lose work), change cost
(the outbox must be replaceable without touching checkout).

\textbf{Accepted damage:} the outbox table is integration-critical and requires operational
monitoring and backup; the polling interval introduces a bounded propagation delay between
order confirmation and warehouse notification; at-least-once delivery requires all consumers
to be idempotent. This decision should be revisited if polling latency grows beyond the
QAS~1 threshold or if a native transactional messaging primitive becomes available in
the nopCommerce platform.
...
\textbf{Dominant forces:} team ownership (each adapter has a clear and isolated scope),
change cost and reversibility (replacing one adapter must not affect others), deployability
(adapters are independently deployable).

\textbf{Accepted damage:} every new external system requires a dedicated adapter
implementation; the adapter layer adds translation code and deployment units that would not
exist under a shared-database approach. This decision should be revisited if the number of
adapters grows beyond what the team can operate independently.
...
\textbf{Dominant forces:} operational complexity (decomposition adds distributed transaction
problems without resolving any identified QAS), deployability (one unit is simpler to run and
test), reversibility (the monolith can be incrementally extracted later if load justifies it).

\textbf{Accepted damage:} the commerce core cannot be scaled independently from its checkout
and order-management data model; all customer-facing capabilities share one release cycle.
This decision should be revisited if QAS~3 load targets require independent scaling of
catalog or order-status reads, or if a new quality attribute requires fault isolation inside
the commerce core itself.
...
\textbf{Dominant forces:} data consistency (duplicate fulfillment or shipping labels cause
operational harm), fault isolation (idempotency must be enforced at the message contract
level, not inside any single adapter).

\textbf{Accepted damage:} every adapter must implement a deduplication check; the
key-outcome table introduces storage overhead proportional to message volume; duplicates
arriving after the 48-hour window will be reprocessed. This decision should be revisited if
message retry windows grow beyond the deduplication window, or if adapter stubs cannot
implement key-level deduplication.
...
by the backoff ceiling and max retries. With an initial delay of 2 seconds, a multiplier of 2,
a maximum delay of 5 minutes, and 10 retry attempts, the accumulated delay before dead-letter
is approximately 18.5 minutes (2\,+\,4\,+\,8\,+\,16\,+\,32\,+\,64\,+\,128\,+\,256\,+\,300\,+\,300\,=\,1{,}110 seconds).
The circuit breaker protects the recovering adapter from burst load. Operators
must be informed when a circuit opens, because it means fulfillment work has stopped flowing
to the affected adapter. This requires a visible circuit-state indicator in the Support and
Operations View; external alerting (email, webhook) is a Phase~4 item documented in the
known limitations.

\textbf{Dominant forces:} fault isolation (a recovering adapter must not be overwhelmed),
supportability (operators must know when a circuit is open and retries have stopped),
operational complexity (backoff parameters and cooldown windows must be tuned per adapter).

\textbf{Accepted damage:} under sustained failure, maximum propagation delay before
dead-letter is approximately 18.5 minutes with production defaults; circuit-breaker state
must be surfaced in the Support and Operations View; backoff parameters introduce
configuration that must be validated during testing. This decision should be revisited if
the dead-letter threshold is too slow for a particular adapter's recovery characteristic.
...
the first and last attempt, and the escalation state (\texttt{New}, \texttt{Requeued}).

The Support and Operations View surfaces dead-letter records alongside pending and retrying
work. Operators can manually re-queue a dead-letter message after the dependency is confirmed
healthy by clicking the requeue action in the Operations View, which creates a new outbox
record from the original payload. External alerting (email, webhook, Slack) when a new
dead-letter entry is created is not implemented in the spike and is deferred to Phase~4
(see known limitations).

Dead-letter records are retained indefinitely in the spike environment. A periodic archival
or deletion policy (for example, 30 days aligned with the order dispute window) is a Phase~4
operational concern and is not implemented.
...
\textbf{Dominant forces:} supportability (operators must be able to observe and act on
permanent failures), data consistency (a paid order must never be silently abandoned),
operational complexity (human intervention must be guided by clear escalation state and
UI affordances).

\textbf{Accepted damage:} permanent failures require a human step in the recovery path, which
introduces latency and operational load; the dead-letter table must be backed up and monitored
as critical infrastructure; external alerting and a retention policy are deferred to Phase~4.
This decision should be revisited if the volume of dead-letter entries grows to a level that
makes manual review unsustainable, at which point automated triage or a tiered escalation
model may be needed.
