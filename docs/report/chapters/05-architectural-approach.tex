...
\hline
Consistency & Stale-state marking, explicit reconciliation & Inventory metadata fields (\texttt{IsStale}, \texttt{ConflictFlag}); reconciliation record created on divergence; the Integration Worker writes the conservative (lower) quantity to \texttt{Product.StockQuantity} when a conflict is detected, which bounds the checkout stock gate indirectly. \\
\hline
