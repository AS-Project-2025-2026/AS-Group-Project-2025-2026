...
\textbf{The runtime evidence exercises the real WMS/POS payload shape.} During evidence
capture, the inventory stub returned WMS stock as \texttt{wmsQuantity} and optional POS stock
as \texttt{posQuantity}. The Integration Worker now records the corresponding
\texttt{SourceSystem} values instead of assuming a single \texttt{quantity} field, so the
staleness and conflict evidence is produced from the same payload shape used by the demo
stubs.
