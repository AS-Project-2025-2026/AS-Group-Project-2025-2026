...
\nolinkurl{Nop.Services/Orders/OrderProcessingService.cs}
ends its successful path with the following sequence (lines 1608--1621):
...
The \texttt{PublishAsync} method in
\nolinkurl{Nop.Services/Events/EventPublisher.cs}
resolves registered consumers and dispatches each one in sequence:
...
The \texttt{ExecuteAsync} method in
\nolinkurl{Nop.Services/ScheduleTasks/ScheduleTaskRunner.cs}
...
\texttt{Product\allowbreak Warehouse\allowbreak Inventory},
\texttt{Stock\allowbreak Quantity\allowbreak History}, and \texttt{Shipment} ---
