...
POS stock reported through the Store POS stub's \texttt{POST /stock/report} endpoint, which
publishes a \texttt{pos.stock.reported} message to the \texttt{pos.stock.updates}
queue with a quantity diverging from the WMS value by more than the configured tolerance (2 units). &
Yes. The POS stock consumer detects the divergence, sets \texttt{ConflictFlag},
...
\textbf{The runtime evidence exercises separate WMS and POS source paths.} During evidence
capture, the inventory stub returns WMS stock, while the Store POS stub publishes POS stock
reports to RabbitMQ. The Integration Worker records the corresponding \texttt{SourceSystem}
values instead of assuming a single \texttt{quantity} field, so the staleness and conflict
evidence is produced from the same source separation used by the demo stubs.
...
\begin{itemize}
    \item The POS source is still a coursework stub rather than a real vendor integration.
    Conflict detection is triggered via \texttt{POST /stock/report} on the Store POS stub,
    which publishes an asynchronous POS stock event without reproducing a full POS lifecycle.
    ...
    \item Performance measurements under sustained load (QAS~3 response measures) have been
    collected using k6 at 25 virtual users (5$\times$ the baseline of 5 concurrent users).
    Results are recorded in \texttt{docs/evidence/logs/qas3-load-test-results.json}.
    Checkout p95 was 74\,ms (target: $<$2000\,ms) and order-status p95 was 87\,ms
    (target: $<$1000\,ms) over 5{,}072 requests with a 0\% error rate.
    Both QAS~3 thresholds passed by a large margin, confirming that the async design keeps
    the customer path fast under peak load. The test was run on a single Docker host, which
    is a constraint on absolute throughput but not on the latency argument: the structural
    decoupling (no synchronous adapter calls in the checkout path) is what drives the result.
\end{itemize}
...
\hline
System healthy baseline & \path{screenshots/01-system-healthy.png} &
Operations View with no errors, no stale or conflicted inventory. \\
\hline
System degraded state & \path{screenshots/02-system-degraded.png} &
Operations View with degraded metrics; circuit breaker or queue backlog present. \\
\hline
Normal flow & \path{screenshots/05-normal-flow-1.png};
\path{screenshots/05-normal-flow-2.png} &
Outbox records with \texttt{Published} status; all circuit breakers \texttt{Closed}. \\
\hline
RabbitMQ queues & \path{screenshots/06-rabbitmq-queues-1.png};
\path{screenshots/06-rabbitmq-queues-2.png} &
All declared queues present: \texttt{fulfillment.requests}, \texttt{shipping.requests},
\texttt{pos.stock.updates}, \texttt{storeops.requests}. The \texttt{pos.stock.updates}
queue is configured with a broker-side dead-letter exchange; fulfillment and shipping
dead-letter handling uses the database \texttt{DeadLetterRecord} table. \\
\hline
Warehouse failure & \path{screenshots/07-warehouse-failure-cb-open-1.png};
\path{screenshots/07-warehouse-failure-cb-open-2.png} &
Outbox records in \texttt{Retrying} and \texttt{Failed} states; warehouse circuit breaker
in \texttt{Open} state. \\
\hline
Warehouse recovery & \path{screenshots/08-warehouse-recovery1.png};
\path{screenshots/08-warehouse-recovery2.png} &
All circuit breakers returned to \texttt{Closed}; pending records dispatched. \\
\hline
Dead-letter creation & \path{screenshots/09-deadletter-created.png} &
Dead letter record with \texttt{EscalationState = New} after shipping outage exhausts
retries. \\
\hline
Dead-letter requeue & \path{screenshots/10-deadletter-requeued1.png};
\path{screenshots/10-deadletter-requeued2.png} &
Dead letter \texttt{EscalationState = Requeued}; new outbox record \texttt{Published}. \\
\hline
Inventory staleness & \path{screenshots/03-inventory-stale.png} &
\texttt{IsStale = true} on inventory projection rows after threshold exceeded. \\
\hline
Inventory conflict & \path{screenshots/04-inventory-conflict.png} &
\texttt{ConflictFlag = true} on affected rows; checkout quantity bounded to lower value. \\
\hline
Worker logs & \path{logs/warehouse-failure.txt} &
Retry attempts with exponential delay, circuit breaker open and close events. \\
\hline
Checkout non-blocking & \path{logs/checkout-not-blocked.txt} &
Checkout command completes while warehouse integration remains asynchronous. \\
\hline
Inventory staleness log & \path{logs/staleness-evidence.txt} &
\texttt{InventoryProjectionRecord} rows show \texttt{IsStale = 1} and
\texttt{PendingReconciliation = 1} after the 30-second threshold. \\
\hline
Inventory conflict log & \path{logs/conflict-evidence.txt} &
Full end-to-end sequence: Store POS stub publishes \texttt{pos.stock.reported}; worker
detects POS\,=\,3 vs WMS\,=\,115 (diff\,>\,tolerance); logs
\texttt{checkout capped at 3}; \texttt{Product.StockQuantity} written to 3;
DB shows both rows with \texttt{ConflictFlag\,=\,1} and \texttt{PendingReconciliation\,=\,1}. \\
\hline
Warehouse recovery log & \path{logs/warehouse-recovery.txt} &
Half-open probe and circuit close events after the warehouse stub recovers. \\
\hline
QAS~3 load test & \path{logs/qas3-load-test-results.json} &
k6 results at 25 VUs (5$\times$ load): checkout p95 = 74\,ms, order-status p95 = 87\,ms,
0\% errors over 5{,}072 requests. Both QAS~3 thresholds passed. \\
\hline
\end{tabular}
\end{center}
\endgroup
